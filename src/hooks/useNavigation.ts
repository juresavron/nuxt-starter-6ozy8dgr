...
export const useNavigation = () => {
  ...
  // Always call useLanguageStore unconditionally to maintain hook order
  // Using a null state selector to avoid errors in non-client environments
  const languageStore = useLanguageStore(state => state?.language ?? 'sl');
  // Then use the value conditionally
  const language = isClient ? languageStore : 'sl';

  // Make sure we always have a valid value for translations
  ...
  const tabs = useMemo(() => {
    // Use optional chaining and provide default empty object to avoid undefined errors
    const navTranslations = translations?.landing?.navigation || {};
    
    return [
      { id: 'benefits', label: navTranslations.benefits || 'Benefits' },
